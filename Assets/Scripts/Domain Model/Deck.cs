...
public class Deck
{
	...
	public Sprite[] cardFaceSpritesForCuoPai;
	...
	private static Dictionary<string, Sprite> cardDictForCuoPai = new Dictionary<string, Sprite>();
	public Sprite GetCardFaceImageForCuoPai(string cardValue) {
		if (cardDictForCuoPai.ContainsKey (cardValue)) {
			return cardDictForCuoPai [cardValue];
		}

		foreach(Sprite sprite in cardFaceSpritesForCuoPai) {
			//Debug.Log (sprite.name);
			if (sprite.name.Contains (cardValue)) {
				cardDictForCuoPai [cardValue] = sprite;
				return sprite;
			}
		}
